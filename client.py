...
# Configure logging
logging.basicConfig(filename='./logs/client_log.txt', level=logging.INFO, format='%(asctime)s - %(message)s')
...
if __name__ == "__main__":
    ...
    Num_clients = 3  # Set the desired number of threads -  number of clients

    threads = []
    for i in range(Num_clients):
        requests_list= []
        request=  request_generator("GET","key"+str(i))
...
# if __name__ == "__main__":
#     client_port = 8888
#     requests = [request_generator("GET", "key1"), request_generator("GET", "key2"), request_generator("GET", "key3")]

#     total_response_time = 0  # Variable to store the total response time

#     for request in requests:
#         response = send_request(client_port, request)
#         print("Received response: ", response.strip())
...
#     client_port = 8888
#     requests_list = [
#         [request_generator("GET", "key1"), request_generator("GET", "key2"), request_generator("GET", "key3")],
#         # You can add more lists of requests for each simulated client here
#     ]

#     Num_clients = 3  # Set the desired number of threads

#     threads = []
#     for i, requests in enumerate(requests_list[:Num_clients]):  # Limit the number of threads
#         thread = threading.Thread(target=simulate_client, args=(client_port, requests), name="Client"+str(i+1))
#         threads.append(thread)
#         thread.start()
